...
//Implement a member method lessThanValues which returns a Java Array List with all the key value pairs from the tree that have a key that is less than a given key, which is passed as parameter. Recall the private class MapEntry from the BinarySearchTree implements the interface KeyValuePair. The elements in the array list must be ordered in pre-order.
...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		public ArrayList<KeyValuePair<K, V>> lessThanValues(K key) {
			...
			//			preorderAux((BinaryTreeNodeImp) this.root, list);
			//			return list;

			ArrayList<KeyValuePair<K,V>> result = new ArrayList<>();
			lessthanAux(root, result, key);
			return result;
		}

		private void lessthanAux(BinaryTreeNode<MapEntry<K,V>> n, 
				ArrayList<KeyValuePair<K,V>> result, K key) {
			if(n == null) {
				return;
			} else {
				if(keyComparator.compare(n.getValue().getKey(), key) < 0) {
					result.add(n.getValue());
				}
				lessthanAux(n.getLeftChild(), result, key);
				lessthanAux(n.getRightChild(), result, key);
			}
		}


		public  void preorderAux(BinaryTreeNodeImp root,
				ArrayList<KeyValuePair<K, V>> array){
			if(root == null){
				return;
			}
			else{
				array.add((KeyValuePair<K, V>) root.getValue());
				preorderAux((BinaryTreeNodeImp) root.getLeftChild(), array);
				preorderAux((BinaryTreeNodeImp) root.getRightChild(), array);
			}
