...
//						temp = temp.getNext());
	...
	public static  Integer majorityElement(List<Integer> L){
		...
		List<HashtableSC.MapEntry<Integer,Integer>> list = new SinglyLinkedList<HashtableSC.MapEntry<Integer,Integer>>();
		...
		while(it.hasNext()) {
			...
			for (int i = 0; i < list.size(); i++) {
				HashtableSC.MapEntry<Integer,Integer> map = list.get(i);
				
				if (map.getKey().equals(num)) {
					int count = map.getValue() + 1;
					map.setValue(count);
					found = true;
				}
			}
			...
			if (!found) {
				HashtableSC.MapEntry<Integer,Integer> map = new HashtableSC.MapEntry<Integer,Integer>(num, 1);
				list.add(map);
			}
			
		}
		
		int  comp = list.get(0).getValue();
		int key =0;
		for (int i = 0; i < list.size(); i++) {
			int obj = list.get(i).getValue();
			if (obj > comp) {
				comp = obj;
