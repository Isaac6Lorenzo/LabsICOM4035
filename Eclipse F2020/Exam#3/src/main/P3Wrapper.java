...
//Implement a member method inFullPath which determines if a key,value pair  is located on a BST where each of its  ancestors have two children (i.e., left and child children). Do not count the node with the key,value pair as its own ancestor. The method receives as parameter the key of the pair to be searched in the BST. The method return true if the key-value pair lies on such path, or false otherwise. The method returns false if the key is not found on the  BST, or if the key is in the root. 
...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		public boolean inFullPath(K key) {
			// ADD YOUR CODE HERE
			//			return inFullPathAux(this, this.root, key);



//			from whatsapp
			if(this.root.getValue().getKey().equals(key)){
				return false;
			}else if(this.root.getLeftChild() != null && this.root.getRightChild() != null){
				return inFullPathAux(this.root.getLeftChild(), key) || inFullPathAux(this.root.getRightChild(), key);
			}
		...
		//incorrect
		public boolean inFullPathAux(BinaryTreeNode<MapEntry<K,V>> node, K key) {
			//		(BinarySearchTree tree, BinaryTreeNode<MapEntry<K, V>> root, K key){
			//			if(tree.isLeaf(root)){
			//				return false;
			//			}
			//			else{
			//				//visit
			//				if (root.getValue().getKey().equals(key)) {
			//					return true;
			//				}
			//				inFullPathAux(tree, root.getLeftChild(), key);
			//				inFullPathAux(tree, root.getRightChild(), key);
			//			}
			//			
			//			return false;
...
//			from whatsapp
			if(node.getValue().getKey().equals(key)){
				...
				return inFullPathAux(node.getLeftChild(), key) || inFullPathAux(node.getRightChild(), key);
			}else{
				return false;
