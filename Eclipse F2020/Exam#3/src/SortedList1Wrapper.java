...
//Extend the functionality of the CircularSortedDoublyLinkedList by adding a method name nearestNeighbors(). This method receives an object e, and find the nearest neighbors of e. The nearest neighbors are defined as: up to two elements that go immediately before e, and up to two elements immediately after e. The neighbors are returned as a sorted list. If the element e is not present, the method returns an empty list.
//
//For example, if L = {Apu, Bob, Cal, Dan, Ed, Ed, Jim, Kim, Ned}, then a call to L.nearestNeighbors(Ed) returns R = {Cal, Dan, Ed, Jim}. Likewise, a call to L. nearestNeighbors(Kim) returns R = {Ed, Jim, Ned}. 
...
public class SortedList1Wrapper {
	...
	public static class CircularSortedDoublyLinkedList<E> implements SortedList<E> {
		...
		public SortedList<E> nearestNeighbors(E e) {
			// ADD CODE HERE
			SortedList<E> list = new CircularSortedDoublyLinkedList<E>(comparator);
			if (!this.contains(e)) {
				return list;
			}
			Iterator<Node> it = (Iterator<Node>) this.iterator();
			while(it.hasNext()) {
				Node obj = it.next();
				if (e.equals(obj.getData())) {
					E prev = obj.getPrev().getData();
					E next = obj.getNext().getData();
					list.add(next);
					list.add(prev);
					prev = obj.getPrev().getPrev().getData();
					next = obj.getNext().getNext().getData();				
