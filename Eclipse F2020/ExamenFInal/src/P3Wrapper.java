...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		public void preorderAux(BinaryTreeNode node, ArrayList<KeyValuePair<K, V>> L){
			if(node == null){
				return;
			}
			else{
				//L.add((KeyValuePair<K, V>) node.);
				preorderAux(node.getLeftChild(), L);
				preorderAux(node.getRightChild(), L);
			}
		}
